-- 03_advanced_cases/solutions/retention_excluding_day0.sql
-- Retention без Day 0 (PostgreSQL)
WITH cohorts AS (
...
SELECT cohort_week, activity_week, retained_users_week,
       SUM(retained_users_week) OVER (
           PARTITION BY cohort_week ORDER BY activity_week ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW
       ) AS cumulative_retained_users
FROM weekly_retention
ORDER BY cohort_week, activity_week;

-- Retention без событий в день регистрации (Day 0) ClickHouse
...
activity AS (
    ...
    FROM cohorts c
    JOIN events e ON c.user_id = e.user_id
    WHERE e.event_time::date > c.cohort_week  -- исключаем Day 0
),
distinct_activity AS (
    SELECT DISTINCT cohort_week, activity_week, user_id
    FROM activity
